...
export async function POST(request: Request) {
  ...
  try {
    ...
    // 3. Optional: validate JSON files before commit (sanity check)
    try {
      const propertiesPath = path.join(cwd, "content", "properties.json");
      const propsData = await fs.readFile(propertiesPath, "utf-8");
      JSON.parse(propsData); // throws if invalid
    } catch {
      return NextResponse.json({ error: "O arquivo properties.json contém erros de formatação. Cancele e salve novamente." }, { status: 400 });
    }
